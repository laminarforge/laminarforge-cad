...
- Real unconnected items from `lamp_rev_b_controller_route_report`: `123`.
...
- The schema-v2 seed now includes `76` named routes and `26` explicit vias and can capture DRC-clean local top-layer routes, reviewed bottom-layer escapes, and named channel routes, but the remaining nets require deliberate fanout/channel routing, especially `GND` (`13`), `+3V3_ANA` (`3`), `HEATER1_GATE` (`3`), `HEATER1_PWM` (`3`), `I2C_SCL` (`3`), `I2C_SDA` (`3`), `HEATER0_PWM` (`2`), `USB_DN` (`2`), `USB_DP` (`2`), `VIN_PROTECTED` (`2`), thermistor mux/ADC, camera/debug, and interlock nets.
