...
- Real unconnected items from `lamp_rev_b_controller_route_report`: `149`.
...
- The schema-v2 seed now includes `52` named routes and `12` explicit vias and can capture DRC-clean local top-layer routes, reviewed bottom-layer escapes, and named channel routes, but the remaining nets require deliberate fanout/channel routing, especially `GND` (`15`), `I2C_SCL` (`4`), `+3V3` (`3`), `+3V3_ANA` (`3`), `ESP_EN` (`3`), heater drive, USB, thermistor mux/ADC, camera/debug, and interlock nets.
