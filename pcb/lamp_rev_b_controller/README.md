...
- Real unconnected items from `lamp_rev_b_controller_route_report`: `119`.
...
- The schema-v2 seed now includes `80` named routes and `34` explicit vias and can capture DRC-clean local top-layer routes, reviewed bottom-layer escapes, and named channel routes, but the remaining nets require deliberate fanout/channel routing, especially `GND` (`13`), `+3V3_ANA` (`3`), `I2C_SCL` (`3`), `I2C_SDA` (`3`), `3V3_SW` (`2`), `HEATER1_GATE` (`2`), `HEATER1_PWM` (`2`), thermistor mux/ADC, camera/debug, and interlock nets.
