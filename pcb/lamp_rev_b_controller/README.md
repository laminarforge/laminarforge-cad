...
- Real unconnected items from `lamp_rev_b_controller_route_report`: `116`.
...
- The schema-v2 seed now includes `83` named routes and `34` explicit vias and can capture DRC-clean local top-layer routes, reviewed bottom-layer escapes, and named channel routes, but the remaining nets require deliberate fanout/channel routing, especially `GND` (`13`), `+3V3_ANA` (`3`), `I2C_SCL` (`3`), `I2C_SDA` (`3`), regulator switch/bootstrap, thermistor mux/ADC, heater, camera/debug, USB, and interlock nets.
