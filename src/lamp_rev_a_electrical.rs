...
fn write_usb_inrush_startup_handoff(
    ...
    write_usb_inrush_startup_csv(config, outputs)?;
    write_usb_inrush_startup_spice(config, outputs)?;
    Ok(())
}

fn write_usb_inrush_startup_csv(
    config: &ValidationConfig,
    outputs: &ElectricalOutputPaths,
) -> Result<(), Box<dyn Error>> {
...
fn write_usb_inrush_startup_spice(
    config: &ValidationConfig,
    outputs: &ElectricalOutputPaths,
) -> Result<(), Box<dyn Error>> {
    ensure_parent(&outputs.usb_inrush_startup_netlist)?;
    let policy = &config.usb_inrush_startup_policy;
    let five_v_load_ohm = if policy.five_v_load_ma > 0.0 {
        policy.vbus_nominal_v / (policy.five_v_load_ma / 1000.0)
    } else {
        1.0e9
    };
    let three_v_three_load_ohm = if policy.three_v_three_load_ma > 0.0 {
        policy.regulated_3v3_v / (policy.three_v_three_load_ma / 1000.0)
    } else {
        1.0e9
    };
    let final_window_start_ms = (policy.simulation_stop_ms * 0.75).max(policy.timestep_ms);
    let mut spice = String::new();
    writeln!(
        spice,
        "* LaminarForge LAMP Rev A USB/VBUS startup transient check"
    )?;
    writeln!(spice, "* Generated by lamp_rev_a_electrical_validate")?;
    writeln!(
        spice,
        "* This is a first-order USB hot-plug and idealized 3.3 V regulator envelope model, not signoff."
    )?;
    writeln!(
        spice,
        "* check_max_vbus_inrush_ma={:.6}",
        policy.max_vbus_inrush_ma
    )?;
    writeln!(
        spice,
        "* check_max_5v_inrush_ma={:.6}",
        policy.max_5v_inrush_ma
    )?;
    writeln!(
        spice,
        "* check_min_final_3v3_v={:.6}",
        policy.min_final_3v3_v
    )?;
    writeln!(
        spice,
        "* check_max_final_3v3_v={:.6}",
        policy.max_final_3v3_v
    )?;
    writeln!(
        spice,
        "* check_regulated_3v3_v={:.6}",
        policy.regulated_3v3_v
    )?;
    writeln!(
        spice,
        "* check_max_3v3_overshoot_v={:.6}",
        policy.max_3v3_overshoot_v
    )?;
    writeln!(spice, "* check_max_startup_ms={:.6}", policy.max_startup_ms)?;
    writeln!(
        spice,
        "VUSB USB_SRC 0 PULSE(0 {:.6} 0 0.001m 0.001m {:.6}m {:.6}m)",
        policy.vbus_nominal_v,
        policy.simulation_stop_ms,
        policy.simulation_stop_ms * 2.0
    )?;
    writeln!(spice, "VVBUS USB_SRC USB_FEED 0")?;
    writeln!(
        spice,
        "RUSB USB_FEED VBUS {:.6}",
        policy.usb_source_resistance_ohm
    )?;
    writeln!(spice, "CVBUS VBUS 0 {:.6}u IC=0", policy.vbus_bulk_cap_u)?;
    writeln!(spice, "DVBUS VBUS DIODE_OUT SS34")?;
    writeln!(spice, "V5SENSE DIODE_OUT DIODE_SENSE 0")?;
    writeln!(
        spice,
        "RDIOD DIODE_SENSE P5V {:.6}",
        policy.schottky_series_resistance_ohm
    )?;
    writeln!(spice, "C5V P5V 0 {:.6}u IC=0", policy.five_v_bulk_cap_u)?;
    writeln!(spice, "R5LOAD P5V 0 {:.6}", five_v_load_ohm)?;
    writeln!(
        spice,
        "V3REG P3SRC 0 PULSE(0 {:.6} 0 0.001m 0.001m {:.6}m {:.6}m)",
        policy.regulated_3v3_v,
        policy.simulation_stop_ms,
        policy.simulation_stop_ms * 2.0
    )?;
    writeln!(spice, "V3SENSE P3SRC P3FEED 0")?;
    writeln!(
        spice,
        "R3REG P3FEED P3V3 {:.6}",
        policy.regulator_series_resistance_ohm
    ...
    writeln!(
        spice,
        "C3V3 P3V3 0 {:.6}u IC=0",
        policy.three_v_three_bulk_cap_u
    )?;
    writeln!(spice, "R3LOAD P3V3 0 {:.6}", three_v_three_load_ohm)?;
    writeln!(spice, ".model SS34 D(Is=2u Rs=0.050 N=1.2 Cjo=120p)")?;
    writeln!(
        spice,
        ".tran {:.6}m {:.6}m uic",
        policy.timestep_ms, policy.simulation_stop_ms
    ...
    writeln!(spice, ".control")?;
    writeln!(spice, "run")?;
    writeln!(spice, "meas tran vbus_inrush_pos_a MAX i(vvbus)")?;
    writeln!(spice, "meas tran vbus_inrush_neg_a MIN i(vvbus)")?;
    writeln!(spice, "meas tran five_v_inrush_pos_a MAX i(v5sense)")?;
    writeln!(spice, "meas tran five_v_inrush_neg_a MIN i(v5sense)")?;
    writeln!(
        spice,
        "meas tran three_v_three_startup_time_s WHEN v(p3v3)={:.6} RISE=1",
        policy.min_final_3v3_v
    )?;
    writeln!(
        spice,
        "meas tran three_v_three_final_v AVG v(p3v3) FROM={final_window_start_ms:.6}m TO={:.6}m",
        policy.simulation_stop_ms
    )?;
    writeln!(spice, "meas tran three_v_three_max_v MAX v(p3v3)")?;
    writeln!(
        spice,
        "meas tran vbus_final_v AVG v(vbus) FROM={final_window_start_ms:.6}m TO={:.6}m",
        policy.simulation_stop_ms
    )?;
    writeln!(
        spice,
        "meas tran five_v_final_v AVG v(p5v) FROM={final_window_start_ms:.6}m TO={:.6}m",
        policy.simulation_stop_ms
    )?;
    writeln!(spice, ".endc")?;
    writeln!(spice, ".end")?;
    fs::write(&outputs.usb_inrush_startup_netlist, spice)?;
    Ok(())
}
...
fn write_simulation_handoff(
    ...
    writeln!(
        report,
        "- USB/VBUS hot-plug and 3.3 V startup transient netlist: `{}`",
        outputs
            .usb_inrush_startup_netlist
            .file_name()
            .and_then(|name| name.to_str())
            .unwrap_or("lamp_rev_a_usb_inrush_startup.spice")
    )?;
    writeln!(
        report,
