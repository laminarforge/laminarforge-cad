...
fn to_grid(mm: f64) -> i32 { (mm / GRID_RES).round() as i32 }
fn to_grid_ceil(mm: f64) -> i32 { (mm / GRID_RES).ceil() as i32 }
fn to_mm(g: i32) -> f64 { g as f64 * GRID_RES }
fn in_bounds(x: i32, y: i32) -> bool { x >= 0 && y >= 0 && (x as usize) < GW && (y as usize) < GH }
fn cell_idx(x: i32, y: i32, layer: usize) -> usize { layer * GW * GH + y as usize * GW + x as usize }

#[derive(Clone, Copy, PartialEq, Eq, Hash)]
struct Cell { x: i32, y: i32, layer: usize }

#[derive(Clone, Copy, Eq, PartialEq)]
struct State { f: u32, cell: Cell }
impl Ord for State { fn cmp(&self, o: &Self) -> Ordering { o.f.cmp(&self.f) } }
impl PartialOrd for State { fn partial_cmp(&self, o: &Self) -> Option<Ordering> { Some(self.cmp(o)) } }

struct Grid { blocked: Vec<bool> }

impl Grid {
    fn new() -> Self { Grid { blocked: vec![false; TOTAL_CELLS] } }
    fn is_blocked(&self, c: Cell) -> bool {
        if !in_bounds(c.x, c.y) { return true; }
        self.blocked[cell_idx(c.x, c.y, c.layer)]
    }
    fn set(&mut self, x: i32, y: i32, layer: usize, val: bool) {
        if in_bounds(x, y) { self.blocked[cell_idx(x, y, layer)] = val; }
    }
    ...
    fn block_pad(&mut self, comp: &Component, pad: &super::Pad, clearance: f64) {
        ...
        let (w, h) = if comp.rotation.abs() > 0.01 {
            ...
            (d, d)
        } else { (pad.width, pad.height) };
        let on_f = pad.layers.contains("F.Cu");
        let on_b = pad.layers.contains("B.Cu");
        let thru = pad.drill.is_some();
        if on_f || thru { self.block_rect(LAYER_F, ax, ay, w, h, clearance); }
        if on_b || thru { self.block_rect(LAYER_B, ax, ay, w, h, clearance); }
    }
    fn unblock_pad(&mut self, comp: &Component, pad: &super::Pad) {
        ...
        for yy in y0..=y1 { for xx in x0..=x1 {
            if on_f || thru { self.set(xx, yy, LAYER_F, false); }
            if on_b || thru { self.set(xx, yy, LAYER_B, false); }
        }}
    }
}

struct TraceSegment { x1: f64, y1: f64, x2: f64, y2: f64, layer: usize, width: f64 }
...
fn build_grid(components: &[Component], exclude_nets: &HashSet<u32>) -> Grid {
    ...
    for y in 0..GH as i32 { for x in 0..GW as i32 {
    ...
    // GND stitching vias
    for &(x, y) in &[
        (83.0,2.0),(97.0,2.0),(0.5,20.0),(93.0,20.0),(0.5,45.0),(93.0,46.0),
        (93.0,47.0),(97.0,47.0),(3.0,74.0),(50.0,74.0),(97.0,74.0),(50.0,78.0),
    ] {
...
fn route_two_pins(grid: &Grid, src: Cell, dst: Cell,
                  src_zone: &HashSet<usize>, dst_zone: &HashSet<usize>) -> Option<Vec<Cell>> {
    let h = |c: Cell| -> u32 {
    ...
    g_cost[src_idx] = 0;
    heap.push(State { f: h(src), cell: src });
    ...
    while let Some(State { f, cell }) = heap.pop() {
        ...
        let g = g_cost[ci];
        if f > g.saturating_add(h(cell)) { continue; }

        for (dx, dy) in [(1i32,0i32),(-1,0),(0,1),(0,-1)] {
            let nx = cell.x + dx;
            let ny = cell.y + dy;
            if !in_bounds(nx, ny) { continue; }
            let next = Cell { x: nx, y: ny, layer: cell.layer };
            let ni = cell_idx(nx, ny, cell.layer);
            // Allow movement through source and destination pad zones even if blocked
            let passable = !grid.is_blocked(next) || dst_zone.contains(&ni) || src_zone.contains(&ni);
            if !passable { continue; }
            let new_g = g + 1;
            if new_g < g_cost[ni] {
                g_cost[ni] = new_g;
                prev[ni] = ci as u32;
                heap.push(State { f: new_g + h(next), cell: next });
            }
        }
        // Via
        let ol = 1 - cell.layer;
        let vc = Cell { x: cell.x, y: cell.y, layer: ol };
        ...
        if via_passable {
            ...
            if new_g < g_cost[vi] {
                g_cost[vi] = new_g;
                prev[vi] = ci as u32;
                heap.push(State { f: new_g + h(vc), cell: vc });
            }
        }
    }
    None
...
fn pad_zone_inner(comp: &Component, pad: &super::Pad, clearance: f64) -> HashSet<usize> {
    ...
    for yy in y0..=y1 { for xx in x0..=x1 {
        if on_f || thru { zone.insert(cell_idx(xx, yy, LAYER_F)); }
        if on_b || thru { zone.insert(cell_idx(xx, yy, LAYER_B)); }
    }}
    zone
}
...
fn path_to_segments(path: &[Cell], width: f64) -> (Vec<TraceSegment>, Vec<(f64, f64)>) {
    ...
    if path.len() < 2 { return (segs, vias); }
        ...
        if p.layer != c.layer {
            ...
            if start < i - 1 || (start == i - 1 && (path[start].x != p.x || path[start].y != p.y)) {
                ...
                segs.push(TraceSegment {
                    x1: to_mm(s.x), y1: to_mm(s.y),
                    x2: to_mm(p.x), y2: to_mm(p.y),
                    layer: s.layer, width,
                });
            }
...
fn mark_trace(grid: &mut Grid, seg: &TraceSegment) {
    ...
    if (seg.x1 - seg.x2).abs() < 0.001 {
        ...
        let xw = to_grid_ceil(hw);
        for y in y0..=y1 { for dx in -xw..=xw { grid.set(x + dx, y, layer, true); } }
        ...
        let yw = to_grid_ceil(hw);
        for x in x0..=x1 { for dy in -yw..=yw { grid.set(x, y + dy, layer, true); } }
    } else {
        let x0 = to_grid(seg.x1.min(seg.x2) - hw);
        let x1 = to_grid(seg.x1.max(seg.x2) + hw);
        let y0 = to_grid(seg.y1.min(seg.y2) - hw);
        let y1 = to_grid(seg.y1.max(seg.y2) + hw);
        for y in y0..=y1 { for x in x0..=x1 { grid.set(x, y, layer, true); } }
    }
}
...
fn collect_nets(components: &[Component]) -> Vec<NetRoute> {
    ...
    for comp in components {
        for (i, pad) in comp.pads.iter().enumerate() {
            if pad.net_id == NET_UNCONNECTED || pad.net_id == NET_GND { continue; }
            let (ax, ay) = pad_abs_pos(comp, i);
            let layer = if pad.layers.contains("F.Cu") { LAYER_F } else { LAYER_B };
            pin_map.entry(pad.net_id).or_default().push((ax, ay, layer, pad.net_name));
        }
    }
    let power: HashSet<u32> = [NET_12V, NET_12V_RAW, NET_5V, NET_3V3, NET_VBUS].iter().copied().collect();
    let mut nets: Vec<NetRoute> = pin_map.into_iter()
        .filter(|(_, pins)| pins.len() >= 2)
        .map(|(id, pins)| {
            let name = pins[0].3;
            let coords: Vec<_> = pins.iter().map(|p| (p.0, p.1, p.2)).collect();
            NetRoute { net_id: id, net_name: name, pins: coords, is_power: power.contains(&id) }
        })
        .collect();
...
pub fn write_manual_connections(pcb: &mut String, components: &[Component]) {
    ...
    // F.Cu: south to D9 anode
    write_trace(pcb, 88.0, jy, 88.0, d9y, pw, "F.Cu", NET_HEATER_P, "HEATER_P");
    write_trace(pcb, 88.0, d9y, d9x, d9y, pw, "F.Cu", NET_HEATER_P, "HEATER_P");
    // F.Cu: branch north to Q1 drain
    write_trace(pcb, 88.0, jy, 88.0, 14.0, pw, "F.Cu", NET_HEATER_P, "HEATER_P");
    write_trace(pcb, 88.0, 14.0, qx, 14.0, pw, "F.Cu", NET_HEATER_P, "HEATER_P");
...
pub fn write_autorouted_traces(pcb: &mut String, components: &[Component]) {
    println!("\n── Autorouter ──");
    println!("Grid: {}x{}x2 = {} cells ({:.1} MB)",
        GW, GH, TOTAL_CELLS, TOTAL_CELLS as f64 / 1024.0 / 1024.0);
    ...
    println!("Nets to route: {} ({} skipped)", routeable.len(), nets.len() - routeable.len());
    ...
    for net in &routeable {
        let width = if net.is_power { POWER_TRACE_WIDTH } else { SIGNAL_TRACE_WIDTH };

        // Unblock this net's pads
        for comp in components {
            for pad in &comp.pads {
                if pad.net_id == net.net_id { grid.unblock_pad(comp, pad); }
            }
        }
        ...
        let mut connected: Vec<(f64, f64, usize, usize)> = vec![(net.pins[0].0, net.pins[0].1, net.pins[0].2, 0)];
        let mut remaining: Vec<(f64, f64, usize, usize)> = net.pins[1..].iter().enumerate()
            .map(|(i, p)| (p.0, p.1, p.2, i + 1)).collect();
        ...
        while !remaining.is_empty() {
            ...
            for (ri, rp) in remaining.iter().enumerate() {
                for (ci, cp) in connected.iter().enumerate() {
                    ...
                    if d < best_d { best_d = d; best_ri = ri; best_ci = ci; }
                }
            }
            let target = remaining.remove(best_ri);
            let source = connected[best_ci];

            let src = Cell { x: to_grid(source.0), y: to_grid(source.1), layer: source.2 };
            let dst = Cell { x: to_grid(target.0), y: to_grid(target.1), layer: target.2 };
            ...
            match route_two_pins(&grid, src, dst, &src_zone, dst_zone) {
                Some(path) => {
                    ...
                    for s in &segs { mark_trace(&mut grid, s); }
                    for &(vx, vy) in &vias { mark_via(&mut grid, vx, vy); }
                    all_segs.extend(segs);
                    all_vias.extend(vias);
                    connected.push(target);
                }
                None => {
                    eprintln!("  FAIL: {} ({:.1},{:.1})->({:.1},{:.1})",
        ...
        for s in &all_segs {
            write_trace(pcb, s.x1, s.y1, s.x2, s.y2, s.width, ln(s.layer), net.net_id, net.net_name);
        }
        ...
        if success {
            routed += 1;
            println!("  {} [{}p]: {} segs, {} vias", net.net_name, net.pins.len(), all_segs.len(), all_vias.len());
        } else {
            failed += 1;
        }
        ...
        for comp in components {
            for pad in &comp.pads {
                if pad.net_id == net.net_id { grid.block_pad(comp, pad, MIN_CLEARANCE); }
            }
        }
    }

    println!("\nResult: {}/{} routed, {} failed, {} segments, {} vias",
        routed, routeable.len(), failed, total_segs, total_vias);
