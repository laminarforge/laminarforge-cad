...
fn build_xy_carriage_plate() {
    ...
    let fw1 = centered_cylinder("fw1", wheel_bore_d / 2.0, plate_t + 2.0, 24).translate(
        -wheel_x_spacing / 2.0,
        wheel_y_fixed,
        0.0,
    );
    let fw2 = centered_cylinder("fw2", wheel_bore_d / 2.0, plate_t + 2.0, 24).translate(
        wheel_x_spacing / 2.0,
        wheel_y_fixed,
        0.0,
    );
    ...
    let ew1 = centered_cylinder("ew1", eccentric_bore_d / 2.0, plate_t + 2.0, 24).translate(
        -wheel_x_spacing / 2.0,
        wheel_y_eccentric,
        0.0,
    );
    let ew2 = centered_cylinder("ew2", eccentric_bore_d / 2.0, plate_t + 2.0, 24).translate(
        wheel_x_spacing / 2.0,
        wheel_y_eccentric,
        0.0,
    );
    ...
    let belt_screw_1 = centered_cylinder("bs1", 1.6, plate_t + 2.0, 24).translate(
        -(plate_w / 2.0) + belt_slot_l + 5.0,
        belt_y,
        0.0,
    );
    let belt_screw_2 = centered_cylinder("bs2", 1.6, plate_t + 2.0, 24).translate(
        plate_w / 2.0 - belt_slot_l - 5.0,
        belt_y,
        0.0,
    );
    ...
    carriage.write_stl("output/xy_carriage_plate.stl").unwrap();
...
fn build_z_carriage() {
    ...
    let vw1 = centered_cylinder("vw1", wheel_bore / 2.0, body_t + 2.0, 24).translate(
        wheel_x_fixed,
        -wheel_y_spacing / 2.0,
        0.0,
    );
    let vw2 = centered_cylinder("vw2", wheel_bore / 2.0, body_t + 2.0, 24).translate(
        wheel_x_fixed,
        wheel_y_spacing / 2.0,
        0.0,
    );
    let vw3 =
        centered_cylinder("vw3", ecc_bore / 2.0, body_t + 2.0, 24).translate(wheel_x_ecc, 0.0, 0.0);
    ...
    let motor_plate = centered_cube("motor_plate", motor_plate_w, motor_plate_h, motor_plate_t)
        .translate(
    ...
    // Boss recess
    let boss = centered_cylinder("boss", (NEMA17_BOSS_DIAMETER + 0.5) / 2.0, 2.5 + 0.1, 48)
        .translate(motor_center_x, 0.0, body_t / 2.0 + 2.5 / 2.0 - 0.05);
    ...
    let clamp_holes = {
        let h1 = centered_cylinder("clamp_1", 1.6, body_t + 2.0, 24).translate(
            body_w / 2.0 - 5.0,
            -clamp_hole_spacing / 2.0,
            0.0,
        );
        let h2 = centered_cylinder("clamp_2", 1.6, body_t + 2.0, 24).translate(
            body_w / 2.0 - 5.0,
            clamp_hole_spacing / 2.0,
            0.0,
        );
    ...
    println!(
        "  Guide rod bore: {:.1}mm",
        SYRINGE_GUIDE_ROD_DIAMETER + 0.1
    );
}
...
fn build_syringe_clamp() {
    ...
    let bolt2 = centered_cylinder("bolt_2", 1.6, clamp_d + 2.0, 24)
        ...
        .translate(
            0.0,
            0.0,
            bolt_spacing / 2.0 + clamp_h / 2.0 - 5.0 - bolt_spacing,
        );
    ...
    println!(
        "  Flange slot:    {flange_slot_w:.1}mm x {:.1}mm",
        flange_t + 0.5
    );
