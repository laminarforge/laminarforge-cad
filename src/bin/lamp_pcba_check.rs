...
fn validate_kicad_seed(
    ...
    if !board.contains("laminarforge_lamp_rev_a_materializer") {
        errors.push("KiCad board must be generated by lamp_rev_a_materialize_board".to_string());
    }
    if board.contains("REF**") {
        errors.push("KiCad board still contains placeholder footprint references".to_string());
    }
    for net in nets {
        if !board.contains(&format!(r#""{}""#, net.name)) {
            errors.push(format!("KiCad board net table is missing {}", net.name));
        }
    }

    for group in &contract.net_groups {
        for index in 0..group.count {
            let net = format!("{}{}", group.prefix, index);
            if !board.contains(&format!(r#""{net}""#)) {
                errors.push(format!("KiCad board net table is missing {net}"));
            }
        }
    }

    for item in &placement.placements {
        if !board.contains(&format!(r#"(property "Reference" "{}""#, item.reference)) {
            errors.push(format!(
                "KiCad board is missing placed footprint {}",
                item.reference
            ));
        }
    }
    ...
    for point in &placement.test_points {
        if !board.contains(&format!(r#"(property "Reference" "{}""#, point.name)) {
            errors.push(format!(
                "KiCad board is missing test point footprint {}",
                point.name
            ));
        }
    }
    ...
    let footprint_count = board.matches("(footprint ").count();
    let expected_minimum = placement.placements.len() + placement.test_points.len();
    if footprint_count < expected_minimum {
        errors.push(format!(
            "KiCad board has {footprint_count} footprints but expected at least {expected_minimum}"
        ));
    }
}
