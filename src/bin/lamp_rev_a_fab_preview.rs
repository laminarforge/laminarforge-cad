...
const OUTPUT_DIR: &str = "pcb/lamp_rev_a/fab";
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("Wrote LAMP Rev A fab preview:");
    println!("  {}/bom.csv", OUTPUT_DIR);
    println!("  {}/cpl.csv", OUTPUT_DIR);
    println!("Use KiBot/KiKit from the materialized KiCad PCB for final fab release.");
    Ok(())
}
...
fn validate(parts: &PartsManifest, placement: &PlacementPlan) -> Result<(), Box<dyn Error>> {
    ...
    for item in &placement.placements {
        ...
        if item.side != "top" {
            errors.push(format!(
                "placement {} is {}, but Rev A fab preview supports top-side assembly only",
