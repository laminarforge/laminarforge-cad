...
This plan is for a small, self-contained starter setup that can support practice mammalian cell culture, early chip seeding, media changes, and basic incubation before the larger 100-chip automation stack is needed.
...
| --- | --- | --- |
| Starter CO2 incubator cabinet | `co2_incubator` | Small humidified 37 C / 5% CO2 chamber |
...
| Incubation | CO2 incubator or starter DIY CO2 cabinet | Build or buy used | Required | Starter cabinet is acceptable for non-critical practice and chip hardware shakeout. |
...
Use `src/bin/co2_incubator.rs` as the first cabinet to build. It is intentionally smaller than `chip_incubator_v3`; the goal is to validate chamber sealing, heat control, CO2 injection, humidity behavior, and workflow ergonomics before scaling up.
...
| CO2 | 5%, validate with independent meter if used for culture |
...
| Gas/sensor manifold | `co2_incubator_service_manifold.stl` | Rear printed bulkhead for CO2 inlet, sensor, cable gland, and heater leads. |
...
| CO2 sensing | NDIR CO2 sensor | 1 | Use a sensor with a range suitable for 5% CO2. Many cheap room-air modules top out too low. |
...
| CO2 supply | Small CO2 cylinder + low-pressure regulator | 1 | Paintball hardware may work for prototypes, but use proper regulator and leak checks. |
...
2. Commissioning protocol: empty-chamber thermal rise, overshoot, door-open recovery, 24-hour stability, CO2 leak/decay test.
3. Still-air-box revision: make the practice enclosure easier to disinfect and add a pass-through tray.
4. Syringe pump fluid path holders: add disposable tubing clips and chip priming fixture.
5. Larger incubator decision: only move to `chip_incubator_v3` after the small cabinet validates control and sealing assumptions.
