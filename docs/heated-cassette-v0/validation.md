...
The current metal-span and required-power results are recorded in thermal-sizing.json for each conductivity/loss case. These are screening estimates, not evidence that the fluid uniformity target is achieved. Rev D bolted-joint contact resistance is not resolved. Map real water temperatures and room drafts; the warmest metal location and its offset from the rear RTD can differ between configurations.
